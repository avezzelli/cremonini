...
class BrandView extends PrinterView implements InterfaceView {
    ...
    public function listenerClienteSaveForm(){
        if(isset($_POST[FRM_SAVE.FRM_CLIENTE])){
            ...
            if(isset($_POST[FRM_CLIENTE_PASS]) && $_POST[FRM_CLIENTE_PASS] != ''){
                ...
                $idUWp = $this->utente->createUtenteWP($cliente->getEmail(), $_POST[FRM_CLIENTE_PASS], RUOLO_CLIENTE, $cliente->getRagioneSociale());
    ...
    public function listenerClienteDetailsForm(){
        ...
        if(isset($_POST[FRM_UPDATE.FRM_CLIENTE])){
            ...
            if($cliente->getUtenteWp() != null ){
                if($_POST[FRM_CLIENTE_PASS] != ''){
                    //c'è il capo user name e la password non è vuota
                    //Devo aggiornare il campo password
                    $this->utente->updatePasswordUtenteWP($_POST[FRM_CLIENTE_PASS], $cliente->getUtenteWp());
                }
                //aggiorno anche la ragione sociale sull'utente wp
                $this->utente->aggiornaUtenteWP($cliente->getUtenteWp(), $cliente->getRagioneSociale());                
            }
    ...
    public function printBrandsByIdCliente($idCliente){
        if($idCliente != null){
            ...
            echo '<p>&Egrave; necessario prima avere configurato il proprietario</p>';
        }
    }
    ...
    public function printResponsabiliCliente($idCliente){        
        if($idCliente != null){
            //stampo il form di salva responsabili
            echo '<strong class="titolo">Lista responsabili presenti</strong>';
            //lista dei responsabili già inseriti
            $this->printResponsabileTableResult($this->utente->getResponsabiliCliente());
            echo '<hr>';
            echo '<strong class="titolo">Aggiungi responsabile</strong>';
            $this->printResponsabileSaveForm();
        }
        else{
            echo '<p>&Egrave; necessario avere prima configurato il proprietario.</p>';
        }
    }
    ...
    protected function printResponsabileTableResult($array){
        if(checkResult($array)){
            $header = array('Cognome e nome', LBL_TELEFONO, LBL_EMAIL, 'AZIONI');
            $rows = array();
            
            foreach($array as $item){
                $utenteC = updateToUtenteC($item);
                $colonne = array(
                    $utenteC->getCognome().' '.$utenteC->getNome(),
                    $utenteC->getTelefono(),
                    $utenteC->getEmail(),
            ...
            echo '<p>Non sono presenti ancora dei responsabili</p>';
        }
    }
