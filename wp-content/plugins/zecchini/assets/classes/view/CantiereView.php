...
class CantiereView extends PrinterView implements InterfaceView {
    ...
    protected function printCantiereTableResult($array){
        if(checkResult($array)){
            $header = array(LBL_NOME, LBL_CANTIERE_INDIRIZZO, LBL_STATO, 'Precollaudo', 'Collaudo', 'AZIONI');
            $rows = array();
            ...
            foreach($array as $item){
                ...
                if($cantiere->getPrecollaudo() != null){
                    $precollaudo = calcolaCompletamentoCollaudo($cantiere->getPrecollaudo());
                }
                
                if($cantiere->getCollaudo() != null){
                    $collaudo = calcolaCompletamentoCollaudo($cantiere->getCollaudo());
                }
    ...
    protected function printDettagliCollaudo(Collaudo $obj, $tipo, $arrayForm, $arrayCantieristi, $idPreCollaudo = null){
        ...
        parent::printStartDetailsForm($tipo);
            echo '<div class="container-dettaglio-collaudo">';
            echo '<div class="grafico-torta">';
            echo '<strong class="titolo">COMPLETAMENTO</strong>';
            echo do_shortcode('[vc_pie value="'.calcolaCompletamentoCollaudo($obj, true).'" label_value="'.calcolaCompletamentoCollaudo($obj, true).'" color="juicy-pink" units="%"]');
            echo '</div>';
        
            echo '<div class="form-dati">';        
            //id collaudo
    ...
    public function printCollaudoLog($idCollaudo){
        ...
        if(checkResult($logs)){
            ...
            echo '<p>Non sono presenti log</p>';
        }
