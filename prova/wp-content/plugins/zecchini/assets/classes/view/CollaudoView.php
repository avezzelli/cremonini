...
class CollaudoView  extends PrinterView implements InterfaceView{
    ...
    ?>    
        ...
        <h5 style="padding-left:15px;"><?php echo LBL_VOCE ?></h5>
        <div class="nascondi-container-voci">Nascondi</div>
        ...
        foreach($_POST as $key => $value){
            if(strpos($key, 'count-voce') !== false){
                ...
                if(isset($_POST['id-voce-'.$value])){
                    ...
                    //ottengo altri campi
                    $temp = updateToVoce($this->collaudo->getVoceByID($voce->getID()));
                    //stato
        ...
        if(isset($_POST[FRM_UPDATE.FRM_GV])){
            ...
            if(checkResult($temp)){
                foreach($temp as $item){
                    ...
                    $log = new Log();
                    $log->setIdCollaudo($_GET['ID']); //lo trovo nell'url della pagina
    ...
    public function printBachecaCollaudo(Collaudo $obj){
        ...
        if(checkResult($gvs)){
            ...
            foreach($gvs as $item){
                ...
                if(canReadGV($gv, get_current_user_id(), $isResponsabile)){
                            ...
                            foreach($gv->getVoci() as $item){
                                ...
                                echo '<div class="container-voce">';
                                    ...
                                    echo '<div class="destra">';                                        
                                        ...
                                        $this->printSemafori($voce, $isResponsabile);                                            
                                        echo '</div>';
                                    echo '</div>';
    ...
    private function printSemafori(Voce $voce, $isResponsabile){
