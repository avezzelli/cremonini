...
function canReadGV(GruppoVoce $gv, $idUserWP, $isResponsabile){        
    //se è amministratore, può vedere
    ...
    if(checkResult($result)){    
        foreach($result as $idBrand => $cantieri){
            ...
            foreach($cantieri as $item){
                ...
                if(isUtenteinCollaudo(get_current_user_id(), $pc->getID())){
                ...
                if(isUtenteinCollaudo(get_current_user_id(), $co->getID())){
...
function isUtenteinCollaudo($idWP, $idCollaudo){
    ...
    //non ho trovato alcuna ricorrenza
    return false;
}
